...
void SettingsMenuMode::handleKeyPress(winrt::Windows::System::VirtualKey pressedKey)
{
	...
	case winrt::Windows::System::VirtualKey::Number2:
	{
		...
		else createAlert(L"Must be connected to a Personal Caddie to go to the IMU Settings Mode.", UIColor::Red, m_uiManager.getScreenSize());
		break;
	}
	case winrt::Windows::System::VirtualKey::Number3:
	{
		if (m_connected) newMode = ModeType::CALIBRATION;
		else createAlert(L"Must be connected to a Personal Caddie to go to the Calibration Mode.", UIColor::Red, m_uiManager.getScreenSize());
		break;
	}
	}
