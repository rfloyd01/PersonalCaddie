...
glm::quat MadgwickModified(glm::quat q, float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz, glm::quat h, float delta_t, float beta)
{
    ...
    */
    ...
    s0 = ((-2 * q.y) * (2 * (q.x * q.z - q.w * q.y) - ax)) + ((2 * q.x) * (2 * (q.w * q.x + q.y * q.z) - ay)) + (0 * (2 * (0.5 - q.x * q.x - q.y * q.y) - az));
    s1 = ((2 * q.z) * (2 * (q.x * q.z - q.w * q.y) - ax)) + ((2 * q.w) * (2 * (q.w * q.x + q.y * q.z) - ay)) + ((-4 * q.x) * (2 * (0.5 - q.x * q.x - q.y * q.y) - az));
    s2 = ((-2 * q.w) * (2 * (q.x * q.z - q.w * q.y) - ax)) + ((-2 * q.z) * (2 * (q.w * q.x + q.y * q.z) - ay)) + ((-4 * q.y) * (2 * (0.5 - q.x * q.x - q.y * q.y) - az));
    s3 = ((2 * q.x) * (2 * (q.x * q.z - q.w * q.y) - ax)) + ((2 * q.y) * (2 * (q.w * q.x + q.y * q.z) - ay)) + (0 * (2 * (0.5 - q.x * q.x - q.y * q.y) - az));
...
{
    //A slightly modified take on Madgwick's algorithm, in this version the Y-axis is taken to be vertical instead of the Z-axis to make things run smoothly with OpenGL
    float recipNorm;
    float del_f_0, del_f_1, del_f_2, del_f_3;
    float _2hx, _2hy, _2hz, _4hx, _4hy, _4hz;
    float _2q0mx, _2q0my, _2q0mz, _2q1mx, _2bx, _2by, _2bz, _4bx, _4by, _4bz, _2q0, _2q1, _2q2, _2q3, _2q0q2, _2q2q3, q0q0, q0q1, q0q2, q0q3, q1q1, q1q2, q1q3, q2q2, q2q3, q3q3;

    //convert Gyroscope readings to rad/s
    //TODO: Need to include step here to estimate gyroscope drift to correct gyro measurements
    gx *= 3.14159 / 180.0;
    gy *= 3.14159 / 180.0;
    gz *= 3.14159 / 180.0;
    ...
    float F[6][1];
    //F[0][0] - F[2][0] = f(q, g, a)
    F[0][0] = 2 * (q.w * q.z + q.x * q.y) - ax;
    F[1][0] = 2 * (0.5 - q.x * q.x - q.z * q.z) - ay;
    F[2][0] = 2 * (q.y * q.z - q.w * q.x) - az;

    //F[3][0] - F[5][0] = f(q, b, m)
    F[3][0] = _2bx * (0.5 - q.y * q.y - q.z * q.z) + _2by * (q.w * q.z + q.x * q.y) - mx; //2*b(2)*(0.5 - q(3)^2 - q(4)^2) + 2*b(4)*(q(2)*q(4) - q(1)*q(3)) - Magnetometer(1)
    F[4][0] = _2bx * (q.x * q.y - q.w * q.z) + _2by * (0.5 - q.x * q.x - q.z * q.z) - my; //2*b(2)*(q(2)*q(3) - q(1)*q(4)) + 2*b(4)*(q(1)*q(2) + q(3)*q(4)) - Magnetometer(2)
    F[5][0] = _2bx * (q.w * q.y + q.x * q.z) + _2by * (q.y * q.z - q.w * q.x) - mz; //2*b(2)*(q(1)*q(3) + q(2)*q(4)) + 2*b(4)*(0.5 - q(2)^2 - q(3)^2) - Magnetometer(3)]

    float J[6][4];
    //J[0][0] - J[2][3] = j(q, g)
    J[0][0] = 2 * q.z;  J[0][1] = 2 * q.y;  J[0][2] = 2 * q.x; J[0][3] = 2 * q.w;
    J[1][0] = 0;        J[1][1] = -4 * q.x; J[1][2] = 0;       J[1][3] = -4 * q.z;
    J[2][0] = -2 * q.x; J[2][1] = -2 * q.w; J[2][2] = 2 * q.z; J[2][3] = 2 * q.y;

    //J[3][0] - J[5][3] = j(q, b)
    J[3][0] = _2by * q.z;              J[3][1] = _2by * q.y;              J[3][2] = -_4bx * q.y + _2by * q.x; J[3][3] = -_4bx * q.z + _2by * q.w;
    J[4][0] = -_2bx * q.z;             J[4][1] = _2bx * q.y - _4by * q.x; J[4][2] = _2bx * q.x;               J[4][3] = -_2bx * q.w - _4by * q.z;
    J[5][0] = _2bx * q.y - _2by * q.x; J[5][1] = _2bx * q.z - _2by * q.w; J[5][2] = _2bx * q.w + _2by * q.z;  J[5][3] = _2bx * q.x + _2by * q.y;
    
    //Calculate Del(f) = J' * F
    del_f_0 = (J[0][0] * F[0][0]) + (J[1][0] * F[1][0]) + (J[2][0] * F[2][0]) + (J[3][0] * F[3][0]) + (J[4][0] * F[4][0]) + (J[5][0] * F[5][0]);
    del_f_1 = (J[0][1] * F[0][0]) + (J[1][1] * F[1][0]) + (J[2][1] * F[2][0]) + (J[3][1] * F[3][0]) + (J[4][1] * F[4][0]) + (J[5][1] * F[5][0]);
    del_f_2 = (J[0][2] * F[0][0]) + (J[1][2] * F[1][0]) + (J[2][2] * F[2][0]) + (J[3][2] * F[3][0]) + (J[4][2] * F[4][0]) + (J[5][2] * F[5][0]);
    del_f_3 = (J[0][3] * F[0][0]) + (J[1][3] * F[1][0]) + (J[2][3] * F[2][0]) + (J[3][3] * F[3][0]) + (J[4][3] * F[4][0]) + (J[5][3] * F[5][0]);

    //
    recipNorm = invSqrt(del_f_0 * del_f_0 + del_f_1 * del_f_1 + del_f_2 * del_f_2 + del_f_3 * del_f_3); // normalise step magnitude
    del_f_0 *= recipNorm;
    del_f_1 *= recipNorm;
    del_f_2 *= recipNorm;
    del_f_3 *= recipNorm;
...
glm::quat Floyd(glm::quat q, float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz, float bx, float by, float bz, float delta_t, float beta)
{
    ...
    */
    ...
    return qDot; //TODO: This is wrong, needed to return something though
}
...
void MadgwickAHRSupdate(glm::quat& q_first, glm::quat& q_second, float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz, float sampleFreq, float beta) {
    ...
    float q0 = q_first.w, q1 = q_first.x, q2 = q_first.y, q3 = q_first.z;
    ...
    if ((mx == 0.0f) && (my == 0.0f) && (mz == 0.0f)) {
        MadgwickAHRSupdateIMU(q_first, q_second, gx, gy, gz, ax, ay, az, sampleFreq, beta);
        return;
    }
