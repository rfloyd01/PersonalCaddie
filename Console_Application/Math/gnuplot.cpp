...
{
    int axes_maximum = -axes_size, axes_minimum = axes_size;
    ...
    for (int i = 0; i < ox_data.size(); i++)
    {
        myFile << ox_data[i] << "    " << oy_data[i] << "    " << oz_data[i] << '\n';
    }
    myFile.close();

    myFile.open("golf_ball.dat");
    for (int i = 0; i < gx.size(); i++)
    {
        myFile << gx[i] << "    " << gy[i] << "    " << gz[i] << '\n';
    }
    myFile.close();

    std::string function = "splot 'ellipse.dat' using 1:2:3 with lines, 'test.dat' using 1:2:3 with lines, 'golf_ball.dat' using 1:2:3";
