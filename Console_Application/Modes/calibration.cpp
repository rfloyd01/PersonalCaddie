...
void Calibration::accTest()
{
	...
	{
		clearMessageType(MessageType::BODY);
		addText(MessageType::BODY, { "Lay Sensor flat on table with +Y axis pointing up like shown in the picture to the right. Sensor will accumulate data for 5 seconds. Press enter to continue.", 10, 420, 0.5, {1.0, 1.0, 1.0}, p_graphics->getScreenWidth() / (float)2.0 });
...
void Calibration::accNextStep()
{
	if (glfwGetTime() - cal_time >= 5)
	{
		...
		{
			//tumble order should be [+Y, -X, +Z, +X, -Z, -Y]
			if (cal_stage == 2)
			{
				addText(MessageType::BODY, { "Data collection complete. Rotate sensor so that the positive X-axis is pointing down as shown in the image. Press enter when ready to continue.", 10, 420, 0.5, {1.0, 1.0, 1.0}, p_graphics->getScreenWidth() / (float)2.0 });
				//mode_q = { .707, 0, 0, -.707 }; //changes orientation of chip for rendering purposes
				setClubRotation({ .707, 0, 0, -.707 });
			}
			else if (cal_stage == 3)
			{
				addText(MessageType::BODY, { "Data collection complete. Rotate sensor 90 degrees so that the positive Z-axis is pointing up as shown in the image. Press enter when ready to continue.", 10, 420, 0.5, {1.0, 1.0, 1.0}, p_graphics->getScreenWidth() / (float)2.0 });
			...
			{
				addText(MessageType::BODY, { "Data collection complete. Rotate sensor 90 degrees so that the positive Y-axis is pointing down as shown in the image. Press enter when ready to continue.", 10, 420, 0.5, {1.0, 1.0, 1.0}, p_graphics->getScreenWidth() / (float)2.0 });
...
void Calibration::gyroNextStep()
{
	...
	else if (cal_stage >= 4)
	{
		if (glfwGetTime() - cal_time >= 5)
		{
			...
			if (cal_stage == 4)
			{
				addText(MessageType::BODY, { "Data gathered succesfully. Now the Z-axis gain must be calibrated. Put the sesnor back to its original position, then rotate it 90 degrees left so that it's standing on its side as shown in the image. Press enter.", 10, 420, 0.5, {1.0, 1.0, 1.0}, p_graphics->getScreenWidth() / (float)2.0 });
				gyr_cal[1] = 0; //reset to 0 incase this is not the first calibration attempt, Y data is done first
			...
			{
				addText(MessageType::BODY, { "Data gathered succesfully. Now the X-axis gain must be calibrated. Lay the sensor back on the table, then rotate it 90 degrees towards you so its standing straight up as shown in the image. Press enter to continue.", 10, 420, 0.5, {1.0, 1.0, 1.0}, p_graphics->getScreenWidth() / (float)2.0 });
				gyr_cal[2] = 0; //reset to 0 incase this is not the first calibration attempt, Z data is done second 
			...
			{
				gyr_cal[0] = 0; //reset to 0 incase this is not the first calibration attempt, X data is last
