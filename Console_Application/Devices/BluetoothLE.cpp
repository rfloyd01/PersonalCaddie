...
//PUBLIC FUNCTIONS
//Constructors
BLEDevice::BLEDevice(guid ServiceUUID, uint32_t CharacteristicUUID, IMU* sensor)
{
    service_UUID = ServiceUUID;
    characteristic_UUID = CharacteristicUUID;
    p_sensor = sensor;
    sampleFreq = p_sensor->getFrequency();
...
{
    std::cout << "Connecting to BLE device with the following address (uint_64t): " << bluetoothAddress << std::endl;
    ...
    std::cout << "Succesfully connected to BLE Device, initiating GATT Service Scan" << std::endl;
    Bluetooth::GenericAttributeProfile::GattDeviceServicesResult servicesResult = co_await bleDevice.GetGattServicesAsync(Bluetooth::BluetoothCacheMode::Cached);
    auto heehee = servicesResult.Status();
    std::cout << "Gatt Scan Complete with GattCommunicationStatus Code = " << (int)heehee << std::endl;

    int desired_service, desired_characteristic;
    for (int i = 0; i < servicesResult.Services().Size(); i++)
    {
        uint32_t s = servicesResult.Services().GetAt(i).Uuid().Data1;
        std::cout << std::hex << "Short UUID for service " << i + 1 << " is: " << s << std::endl;
        if (s == service_UUID.Data1) desired_service = i;
    }

    Bluetooth::GenericAttributeProfile::GattDeviceService service = servicesResult.Services().GetAt(desired_service);
    ...
    for (int i = 0; i < characteristicResult.Characteristics().Size(); i++)
    {
        ...
        uint16_t a = characteristicResult.Characteristics().GetAt(i).Uuid().Data2;
        uint16_t b = characteristicResult.Characteristics().GetAt(i).Uuid().Data3;
        uint8_t d1 = characteristicResult.Characteristics().GetAt(i).Uuid().Data4[0];
        uint8_t d2 = characteristicResult.Characteristics().GetAt(i).Uuid().Data4[1];
        uint8_t d3 = characteristicResult.Characteristics().GetAt(i).Uuid().Data4[2];
        uint8_t d4 = characteristicResult.Characteristics().GetAt(i).Uuid().Data4[3];
        uint8_t d5 = characteristicResult.Characteristics().GetAt(i).Uuid().Data4[4];
        uint8_t d6 = characteristicResult.Characteristics().GetAt(i).Uuid().Data4[5];
        uint8_t d7 = characteristicResult.Characteristics().GetAt(i).Uuid().Data4[6];
        uint8_t d8 = characteristicResult.Characteristics().GetAt(i).Uuid().Data4[7];
        //std::cout << std::hex << "Short UUID for characteristic " << i + 1 << " is: " << c << std::endl;
        std::cout << std::hex << "        Full UUID is: " << c << "-" << a << "-" << b << "-" << (int)d1 <<
            (int)d2 << (int)d3 << (int)d4 << (int)d5 << (int)d6 << (int)d7 << (int)d8 << std::endl;
        if (c == characteristic_UUID)
        {
            std::cout << "Desired Characteristic found." << std::endl;
            desired_characteristic = i;
        }
    }
    std::cout << "Done scanning for characteristics" << std::endl;
    Bluetooth::GenericAttributeProfile::GattCharacteristic characteristic = characteristicResult.Characteristics().GetAt(desired_characteristic);
    ...
    if ((characteristic.CharacteristicProperties() & GattCharacteristicProperties::Notify) == GattCharacteristicProperties::Notify)
    {
        ...
        std::cout << "CCCD changed to notify." << std::endl;
            ...
            characteristic.ValueChanged(Windows::Foundation::TypedEventHandler<GattCharacteristic, GattValueChangedEventArgs>([this](GattCharacteristic car, GattValueChangedEventArgs eventArgs)
                {
                    ...
                    Axis correct_axis = X; //initialize the correct_axis variable, this variable is responsible for swapping sensor axes to OpenGL axes
                    ...
                    for (int i = 0; i < number_of_samples; i++)
                    {
                        ...
                        for (int j = 0; j < 3; j++)
                        {
                            ...
                            correct_axis = p_sensor->getOpenGLAxis(Sensor::ACCELEROMETER, j);
                            //r_accelerometer[j][i] = ayyy * acc_conversion; //this is the original code
                            r_accelerometer[correct_axis][i] = ayyy * p_sensor->getSensorSensitivity(Sensor::ACCELEROMETER) * p_sensor->getAxisPolarity(Sensor::ACCELEROMETER, j); //this is the original code
                        }
                        //TODO: remove below line when testing is complete
                        //std::cout << "Raw Acc: {" << r_accelerometer[0][i] << ", " << r_accelerometer[1][i] << ", " << r_accelerometer[2][i] << "}" << std::endl;

                        //Second is gyroscope data
                        for (int j = 0; j < 3; j++)
                        {
                            int16_t ayyy = ((read.ReadByte()) | (read.ReadByte() << 8));
                            correct_axis = p_sensor->getOpenGLAxis(Sensor::GYROSCOPE, j);
                            //r_gyroscope[j][i] = ayyy * gyr_conversion;
                            r_gyroscope[correct_axis][i] = ayyy * p_sensor->getSensorSensitivity(Sensor::GYROSCOPE) * p_sensor->getAxisPolarity(Sensor::GYROSCOPE, j);
                        }
                        //TODO: remove below line when testing is complete
                        //std::cout << "Raw Gyr: {" << r_gyroscope[0][i] << ", " << r_gyroscope[1][i] << ", " << r_gyroscope[2][i] << "}" << std::endl;

                        //Third is magnet data
                        for (int j = 0; j < 3; j++)
                        {
                            int16_t ayyy = ((read.ReadByte()) | (read.ReadByte() << 8));
                            correct_axis = p_sensor->getOpenGLAxis(Sensor::MAGNETOMETER, j);
                            //r_magnetometer[j][i] = ayyy * mag_conversion;
                            r_magnetometer[correct_axis][i] = ayyy * p_sensor->getSensorSensitivity(Sensor::MAGNETOMETER) * p_sensor->getAxisPolarity(Sensor::MAGNETOMETER, j);
                            //std::cout << j << ", " << p_sensor->getAxisPolarity(Sensor::MAGNETOMETER, j) << std::endl;
                        }
                        //TODO: remove below line when testing is complete
                        //std::cout << "Raw Mag: {" << r_magnetometer[0][i] << ", " << r_magnetometer[1][i] << ", " << r_magnetometer[2][i] << "}" << std::endl << std::endl;

                        //THIS BELOW CODE IS ONLY TRUE FOR THE LSM9DS1
                        //r_magnetometer[2][i] *= -1; //need to flip z-mag for the LSM9DS1
                    }
